// mochou-p/editerm/src/main.rs

mod config;
mod utils;
mod view;

use std::io::Write as _;
use std::os::fd::AsRawFd;
use std::path::PathBuf;
use std::panic::{set_hook, take_hook, catch_unwind, AssertUnwindSafe};
use std::sync::OnceLock;
use spliterm::{Screen, FocusDirection, PaneEvent, CollapseResult, PaneView};
use spliterm::betterm;
use betterm::cursor;
use betterm::{reset, color::{ansi, fg, rgb, RgbColor}};
use betterm::terminal::{RawTerminal, Event, MouseEvent, HoverEvent, KeyboardEvent, Key, CtrlAltableChar, size};
use config::Theme;
use view::{Browsing, Editing, Tabs, Welcome};


static PANIC_LOCATION: OnceLock<String> = OnceLock::new();
static PANIC_PAYLOAD:  OnceLock<String> = OnceLock::new();

fn main() {
    let was_ok = {
        Editor::new().run()
    };

    if !was_ok {
        eprintln!(
            "{}{} crashed! panic info:{}\n{}{}",
            fg(ansi::red().bright()),
            env!("CARGO_CRATE_NAME"),
            reset::fg(),
            PANIC_LOCATION.get().unwrap_or(&String::new()),
            PANIC_PAYLOAD .get().unwrap()
        );
    }
}

pub type In  = Option<Theme>;
pub type Out = ColoredText;

#[derive(Clone)]
pub struct ColoredText {
    pub fg:   RgbColor,
    pub text: String
}

impl Default for ColoredText {
    fn default() -> Self {
        Self { fg: rgb(0, 0, 0), text: String::default() }
    }
}

struct Editor {
    themes:   Vec<Theme>,
    theme:    usize,
    terminal: RawTerminal,
    screen:   Screen<ViewEvent, Theme, In, Out>,
    x:        u16,
    y:        u16,
    width:    u16,
    height:   u16
}

#[derive(Default, Clone)]
pub enum ViewEvent {
    #[default]
    DoNothing,
    DrawCursor(u16, u16),
    CloseMe
}

#[derive(Default, Clone)]
pub struct Cursor {
    last_x: isize,
    x:      isize,
    y:      isize
}

impl PartialEq<Self> for Cursor {
    fn eq(&self, other: &Self) -> bool {
        self.x == other.x && self.y == other.y
    }
}

impl Editor {
    fn new() -> Self {
        let themes          = Theme::all();
        let theme           = 0;
        let output          = std::io::stdout();
        let (x, y)          = (0, 0);
        let (width, height) = size(output.as_raw_fd());
        let screen          = Self::screen_from_args(x, y, width, height, themes[theme].pane_separator);
        let terminal        = RawTerminal::new(std::io::stdin(), output);

        Self { themes, theme, terminal, screen, x, y, width, height }
    }

    fn screen_from_args(x: u16, y: u16, width: u16, height: u16, pane_separator: RgbColor) -> Screen<ViewEvent, Theme, In, Out> {
        let args = std::env::args().collect::<Vec<String>>();

        if args.len() == 1 {
            let view = Tabs::new(vec![Box::new(Welcome::new(None))]);
            return Screen::new(0, 0, width, height, view, pane_separator);
        }

        let mut views = Vec::<Box<dyn PaneView<ViewEvent, Theme, In, Out>>>::with_capacity(args.len());

        for arg in &args[1..] {
            let path = PathBuf::from(arg).canonicalize().unwrap();

            if path.is_dir() {
                views.push(Box::new(Browsing::new(Some(path), None)));
            } else if path.is_file() {
                views.push(Editing::new(path, None));
            } else {
                panic!("{}invalid path{}", fg(ansi::red()), reset::fg())
            }
        }

        Screen::new(x, y, width, height, Tabs::new(views), pane_separator)
    }

    fn run(mut self) -> bool {
        set_hook(Box::new(|panic_info| {
            if let Some(location) = panic_info.location() {
                let _ = PANIC_LOCATION.set(
                    format!(
                        "{}:{}:{}:\n",
                        location.file(), location.line(), location.column()
                    )
                );
            }

            let payload = panic_info.payload();

            let _ = PANIC_PAYLOAD.set(
                if let Some(small_string) = payload.downcast_ref::<&str>() {
                    String::from(*small_string)
                } else if let Some(big_string) = payload.downcast_ref::<String>() {
                    big_string.clone()
                } else {
                    String::from("anonymous panic")
                }
            );
        }));

        let result = catch_unwind(AssertUnwindSafe(|| {
            self.inner_run();
        }));

        let _ = take_hook();

        result.is_ok()
    }

    fn inner_run(&mut self) {
        self.screen.event(Event::Custom(PaneEvent::FocusGained), &mut self.terminal, true, &self.themes[self.theme]);
        self.screen.render_all(&mut self.terminal, &self.themes[self.theme]);
        self.terminal.flush().unwrap();

        loop {
            let event = self.terminal.blocking_event();

            if let Event::Keyboard(keyboard_event) = &event {
                match keyboard_event {
                    KeyboardEvent::Ctrl(Key::Escape) => {
                        break;
                    },
                    KeyboardEvent::Alt(Key::ArrowUp) => {
                        if let Some((x, y)) = self.screen.panes.focus_direction(FocusDirection::Up) {
                            let (x, y) = {
                                if
                                    x <   self.x                as i32
                                    ||
                                    x >= (self.x + self.width)  as i32
                                    ||
                                    y <   self.y                as i32
                                    ||
                                    y >= (self.y + self.height) as i32
                                {
                                    continue;
                                } else {
                                    (x as u16, y as u16)
                                }
                            };

                            self.screen.event(
                                Event::Mouse(MouseEvent::Hover(HoverEvent::NoModifiers(x, y))),
                                &mut self.terminal,
                                false,
                                &self.themes[self.theme]
                            );
                            self.screen.render_all(&mut self.terminal, &self.themes[self.theme]);
                            self.terminal.flush().unwrap();
                        }
                        continue;
                    },
                    KeyboardEvent::Alt(Key::ArrowDown) => {
                        if let Some((x, y)) = self.screen.panes.focus_direction(FocusDirection::Down) {
                            let (x, y) = {
                                if
                                    x <   self.x                as i32
                                    ||
                                    x >= (self.x + self.width)  as i32
                                    ||
                                    y <   self.y                as i32
                                    ||
                                    y >= (self.y + self.height) as i32
                                {
                                    continue;
                                } else {
                                    (x as u16, y as u16)
                                }
                            };

                            self.screen.event(
                                Event::Mouse(MouseEvent::Hover(HoverEvent::NoModifiers(x, y))),
                                &mut self.terminal,
                                false,
                                &self.themes[self.theme]
                            );
                            self.screen.render_all(&mut self.terminal, &self.themes[self.theme]);
                            self.terminal.flush().unwrap();
                        }
                        continue;
                    },
                    KeyboardEvent::Alt(Key::ArrowLeft) => {
                        if let Some((x, y)) = self.screen.panes.focus_direction(FocusDirection::Left) {
                            let (x, y) = {
                                if
                                    x <   self.x                as i32
                                    ||
                                    x >= (self.x + self.width)  as i32
                                    ||
                                    y <   self.y                as i32
                                    ||
                                    y >= (self.y + self.height) as i32
                                {
                                    continue;
                                } else {
                                    (x as u16, y as u16)
                                }
                            };

                            self.screen.event(
                                Event::Mouse(MouseEvent::Hover(HoverEvent::NoModifiers(x, y))),
                                &mut self.terminal,
                                false,
                                &self.themes[self.theme]
                            );
                            self.screen.render_all(&mut self.terminal, &self.themes[self.theme]);
                            self.terminal.flush().unwrap();
                        }
                        continue;
                    },
                    KeyboardEvent::Alt(Key::ArrowRight) => {
                        if let Some((x, y)) = self.screen.panes.focus_direction(FocusDirection::Right) {
                            let (x, y) = {
                                if
                                    x <   self.x                as i32
                                    ||
                                    x >= (self.x + self.width)  as i32
                                    ||
                                    y <   self.y                as i32
                                    ||
                                    y >= (self.y + self.height) as i32
                                {
                                    continue;
                                } else {
                                    (x as u16, y as u16)
                                }
                            };

                            self.screen.event(
                                Event::Mouse(MouseEvent::Hover(HoverEvent::NoModifiers(x, y))),
                                &mut self.terminal,
                                false,
                                &self.themes[self.theme]
                            );
                            self.screen.render_all(&mut self.terminal, &self.themes[self.theme]);
                            self.terminal.flush().unwrap();
                        }
                        continue;
                    },
                    KeyboardEvent::CtrlAlt(Key::ArrowUp) => {
                        self.screen.panes.vertical_split(Tabs::new(vec![Box::new(Browsing::new(None, None))]), true);
                        self.screen.render_all(&mut self.terminal, &self.themes[self.theme]);
                        self.terminal.flush().unwrap();
                        continue;
                    },
                    KeyboardEvent::CtrlAlt(Key::ArrowDown) => {
                        self.screen.panes.vertical_split(Tabs::new(vec![Box::new(Browsing::new(None, None))]), false);
                        self.screen.render_all(&mut self.terminal, &self.themes[self.theme]);
                        self.terminal.flush().unwrap();
                        continue;
                    },
                    KeyboardEvent::CtrlAlt(Key::ArrowLeft) => {
                        self.screen.panes.horizontal_split(Tabs::new(vec![Box::new(Browsing::new(None, None))]), true);
                        self.screen.render_all(&mut self.terminal, &self.themes[self.theme]);
                        self.terminal.flush().unwrap();
                        continue;
                    },
                    KeyboardEvent::CtrlAlt(Key::ArrowRight) => {
                        self.screen.panes.horizontal_split(Tabs::new(vec![Box::new(Browsing::new(None, None))]), false);
                        self.screen.render_all(&mut self.terminal, &self.themes[self.theme]);
                        self.terminal.flush().unwrap();
                        continue;
                    },
                    KeyboardEvent::CtrlAltBackspace => {
                        match self.screen.panes.collapse() {
                            CollapseResult::EndFound => {
                                break;
                            },
                            CollapseResult::FocusMe => {
                                let custom = self.screen.event(Event::Custom(PaneEvent::FocusGained), &mut self.terminal, true, &self.themes[self.theme]);
                                if self.custom_event(custom) {
                                    self.screen.render_all(&mut self.terminal, &self.themes[self.theme]);
                                    self.terminal.flush().unwrap();
                                    continue;
                                } else {
                                    break;
                                }
                            },
                            CollapseResult::DoNothing => {
                                continue;
                            }
                        }
                    },
                    KeyboardEvent::CtrlAltChar(CtrlAltableChar::T) => {
                        self.theme                  = (self.theme + 1) % self.themes.len();
                        self.screen.separator_color = self.themes[self.theme].pane_separator;
                        self.screen.render_all(&mut self.terminal, &self.themes[self.theme]);
                        self.terminal.flush().unwrap();
                    },
                    _ => ()
                }
            }

            let custom = self.screen.event(event, &mut self.terminal, true, &self.themes[self.theme]);
            if !self.custom_event(custom) {
                break;
            }
        }
    }

    fn custom_event(&mut self, event: Option<ViewEvent>) -> bool {
        match event {
            Some(ViewEvent::DrawCursor(x, y)) => {
                self.terminal.write_all(
                    format!(
                        "{}{}",
                        cursor::goto(
                            x + 1 + self.screen.panes.focused_x().unwrap(),
                            y + 1 + self.screen.panes.focused_y().unwrap()
                        ),
                        "\x1b[?25h" // show cursor
                    ).as_bytes()
                ).unwrap();
                self.terminal.flush().unwrap();

                true
            },
            Some(ViewEvent::CloseMe) => {
                match self.screen.panes.collapse() {
                    CollapseResult::EndFound => {
                        false
                    },
                    CollapseResult::FocusMe => {
                        let custom = self.screen.event(Event::Custom(PaneEvent::FocusGained), &mut self.terminal, true, &self.themes[self.theme]);
                        if self.custom_event(custom) {
                            self.screen.render_all(&mut self.terminal, &self.themes[self.theme]);
                            self.terminal.flush().unwrap();
                            true
                        } else {
                            false
                        }
                    },
                    CollapseResult::DoNothing => true
                }
            },
            _ => {
                self.terminal.write_all(
                    b"\x1b[?25l" // hide cursor
                ).unwrap();
                self.terminal.flush().unwrap();

                true
            }
        }
    }
}
